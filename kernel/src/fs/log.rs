...
impl SpinLock<Log> {
    ...
    /// 并在日志空间不足或日志正在提交时阻塞当前进程（主线程），直到可以继续为止。
    ///
    /// # 流程解释
    /// 1. 加锁以获取对日志的独占访问权；
    /// 2. 检查是否满足以下任一条件，若满足则阻塞当前进程（主线程）：
    ...
    ///
    /// # 可能的错误
    /// - 若存在日志提交未完成，调用将阻塞当前进程（主线程）；
    /// - 若估算日志空间不足，也将使当前进程（主线程）阻塞；
    /// - 若锁的释放和重新获取过程实现不当，可能导致死锁；
    /// - `sleep` 调用依赖当前进程（主线程）指针存在和可调度，否则可能造成未定义行为。
    ///
    /// # 安全性
    /// 本函数不包含 `unsafe` 块，但依赖对 `SpinLock` 的正确使用和进程（主线程）调度机制的健壮性。
    /// 为保证安全性：
    /// - 调用者应确保在内核上下文中调用；
    ...
    pub fn begin_op(&self) {
        ...
        loop {
                ...
                (guard.outstanding+1) as usize * MAXOPBLOCKS > LOGSIZE
            {
                ...
                unsafe { CPU_MANAGER.my_task().sleep(channel, guard); }
                guard = self.lock();
            } else {
                guard.outstanding += 1;
                drop(guard);

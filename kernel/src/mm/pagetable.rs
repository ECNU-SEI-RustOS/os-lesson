...
impl PageTable {
    ...
    /// 与 [walk_alloc] 功能相同，
    /// 但如果页表不存在时不会分配新的页表。
    pub fn find_pte_mut(&mut self, va: VirtAddr) -> Option<&mut PageTableEntry> {
        let mut pagetable = self as *mut PageTable;
        for level in (1..=2).rev() {
            let pte = unsafe { &mut pagetable.as_mut().unwrap().data[va.page_num(level)] };
    ...
    /// # 功能说明
    /// 分配并初始化一个新的进程（主线程）页表，
    /// 包含对陷阱处理跳板（trampoline）和进程（主线程）陷阱帧（trapframe）内存区域的映射。  
    /// 该函数为新进程（主线程）创建独立的页表页，设置必要的权限以支持用户态与内核态的切换。
    ///
    /// # 参数
    /// - `trapframe`: 传入陷阱帧的物理地址，表示进程（主线程）上下文保存的物理页地址。
    ///
    /// # 返回值
    ...
    /// # 功能说明
    /// 释放进程（主线程）页表中与用户空间相关的虚拟内存映射，  
    /// 包括陷阱跳板（TRAMPOLINE）、陷阱帧（TRAPFRAME）以及进程（主线程）用户空间占用的物理内存。  
    /// 该函数负责撤销映射并释放对应物理内存，通常在进程（主线程）退出或页表销毁时调用。
    ///
    /// # 参数
    /// - `&mut self`：进程（主线程）对应的页表的可变引用。  
    /// - `proc_size`：用户进程（主线程）占用的虚拟地址空间大小（字节）。
    ///
    /// # 返回值
    /// 无返回值。
    ///
    ...
    /// # 功能说明
    /// 初始化进程（主线程）用户空间的第一个内存页（代码页），  
    /// 将传入的程序代码 `code` 拷贝到用户空间的固定起始位置（`USERTEXT`），  
    /// 并建立对应虚拟地址到物理页的映射，权限包括读、写、执行和用户访问。
    ///
    /// # 参数
    /// - `&mut self`：进程（主线程）页表的可变引用。  
    /// - `code`：包含进程（主线程）初始代码的字节切片。
    ///
    /// # 返回值
    ...
    /// - 适合在进程（主线程）创建早期调用，需保证单线程环境或同步机制。
    ...
    /// # 功能说明
    /// 为进程（主线程）用户空间从 `old_size` 扩展到 `new_size` 分配新的内存页，
    /// 并建立对应的虚拟地址到物理页的映射，权限包括读、写、执行和用户访问。  
    /// 如果 `new_size` 小于或等于 `old_size`，则不做任何操作直接返回。  
    /// 该函数负责为进程（主线程）用户空间分配新的页并初始化映射，支持动态扩展内存。
    ///
    /// # 参数
    /// - `&mut self`：进程（主线程）页表的可变引用。  
    ...
    pub fn uvm_unmap(&mut self, va: usize, count: usize, freeing: bool) {
        ...
        for ca in (va..(va + PAGE_SIZE * count)).step_by(PAGE_SIZE) {
            let pte = self
                .find_pte_mut(unsafe { VirtAddr::from_raw(ca) })
            ...
            if freeing {
                ...
                unsafe {
                    RawSinglePage::from_raw_and_drop(pa.into_raw() as *mut u8);
                }
            }
            pte.write_zero();
        }
    ...
    /// # 功能说明
    /// 复制当前页表所管理的用户空间内存到子进程（主线程）的页表 `child_pgt`，  
    /// 实现用户空间内存的逐页深拷贝，常用于进程（主线程）创建（fork）时的地址空间复制。  
    /// 对每个页表项执行内存页克隆，建立子页表对应的映射关系，权限保持一致。
    ///
    /// # 参数
    /// - `&mut self`：当前（父）进程（主线程）的页表可变引用。  
    /// - `child_pgt`：子进程（主线程）的页表可变引用。  
    /// - `size`：需复制的用户空间大小（字节）。
    ///
    /// # 返回值
    /// - `Ok(())`：复制成功。  

...
///
/// 该结构用于管理每个 CPU 上当前运行的进程（主线程）、调度上下文等信息，
...
/// 用于跟踪每个 CPU 的运行状态、调度上下文以及当前运行的进程（主线程）。
///
/// 该结构是多核系统调度的核心组件，负责协调不同 CPU 之间的进程（主线程）调度。
/// 访问和修改时通常需要关闭中断或使用同步机制以避免竞态。
///
pub struct CpuManager {
    /// 所有 CPU 核心状态的数组，长度为系统支持的最大 CPU 数量 `NCPU`。
    /// 每个元素对应一个 CPU 核心，包含该 CPU 的运行进程（主线程）和调度上下文。
    table: [Cpu; NCPU],
}
...
impl CpuManager {
    ...
    /// # 功能说明
    /// 获取当前 CPU 上正在运行的进程（主线程）的可变引用。
    /// 该方法可以在多核系统中被不同 CPU 同时调用，
    /// 用于访问当前 CPU 所调度的进程（主线程）。
    ///
    /// # 参数
    /// - `&self`：`CpuManager` 的不可变引用。
    ///
    /// # 返回值
    /// 返回当前 CPU 上运行的进程（主线程）的可变引用 `&mut Proc`。
    ///
    /// # 可能的错误
    /// - 如果当前 CPU 上没有运行的进程（主线程）（`proc` 指针为空），
    ///   则会触发 panic。
    ///
    /// # 安全性
    ...
    /// # 功能说明
    /// CPU 调度器主循环，实现多核环境下对进程（主线程）的抢占式调度。
    /// 该函数从进程（主线程）管理器中选择一个可运行进程（主线程），进行上下文切换，
    /// 在当前 CPU 上运行选中的进程（主线程）。调度器永不返回，
    /// 通过循环不断调度进程（主线程）执行。
    ///
    /// # 流程解释
    /// 1. 调用 `my_cpu_mut()` 获取当前 CPU 的可变引用。
    /// 2. 进入无限循环，确保设备中断打开以允许硬件中断响应。
    /// 3. 通过 `PROC_MANAGER.alloc_runnable()` 尝试获取一个可运行的进程（主线程）。
    ///    - 若成功，设置当前 CPU 的 `proc` 指针指向该进程（主线程）。
    ///    - 获取该进程（主线程）的排他锁，修改进程（主线程）状态为 `RUNNING`。
    ///    - 调用外部汇编函数 `swtch`，完成从调度器上下文切换到进程（主线程）上下文。
    ///    - 上下文切换返回后，检查 `proc` 是否为空，
    ///      若为空则触发 panic，说明调度异常。
    ///    - 清空 `proc` 指针，释放进程（主线程）锁。
    /// 4. 若无可运行进程（主线程），继续循环等待。
    ///
    /// # 参数
    ...
    ///
    /// # 安全性
    /// - 该函数为 `unsafe`，调用时必须保证当前 CPU 和进程（主线程）状态的正确初始化，
    ///   并且调用环境无竞态。
    /// - 调用期间中断被开启以响应外部事件，需确保中断处理正确。
    /// - 上下文切换依赖外部汇编函数 `swtch`，要求其正确保存和恢复寄存器状态。
    /// - 进程（主线程）的排他锁在调度期间被持有，防止进程（主线程）状态并发修改。
    ///
...
/// 包含当前运行的进程（主线程）指针、调度上下文，以及中断嵌套计数和中断使能状态。
///
/// 该结构体设计为单核独占访问，不需要额外的锁保护，
/// 只由对应 CPU 核心本地访问，确保访问安全性。
///
pub struct Cpu {
    /// 当前在该 CPU 上运行的进程（主线程）的裸指针。
    /// 如果没有运行进程（主线程），则为 null。
    task: Option<*mut Task>,

    /// 调度器上下文，用于保存调度器自身的寄存器状态，
    /// 在进程（主线程）切换时作为切换目标上下文。
    scheduler: Context,
...
impl Cpu {
    ...
    /// # 功能说明
    /// 从当前运行的进程（主线程）上下文切换回调度器上下文。
    /// 该函数在切换期间保持进程（主线程）的锁（`SpinLockGuard`），
    /// 确保进程（主线程）状态的一致性和并发安全。
    ///
    /// # 流程解释
    /// 1. 检查当前持有的锁是否是进程（主线程）锁，确保调用前已加锁。
    /// 2. 确认 CPU 当前只持有一个锁（`noff == 1`），防止多锁竞争。
    /// 3. 验证进程（主线程）状态不是运行中，避免非法调度切换。
    /// 4. 确保中断被禁止，避免调度过程中被中断打断。
    /// 5. 保存当前中断使能状态 `intena`。
    /// 6. 调用外部汇编函数 `swtch`，从传入的进程（主线程）上下文切换到调度器上下文。
    /// 7. 恢复中断使能状态。
    /// 8. 返回持有的进程（主线程）锁。
    ///
    /// # 参数
    /// - `&mut self`：当前 CPU 的可变引用。
    /// - `guard`：持有的进程（主线程）排他锁 `SpinLockGuard`，保证进程（主线程）状态不被并发修改。
    /// - `ctx`：指向当前进程（主线程）上下文的裸指针，用于上下文切换。
    ///
    /// # 返回值
    /// 返回传入的进程（主线程）锁 `SpinLockGuard`，以便调用者继续持有锁。
    ///
    /// # 可能的错误
    /// - 若未持有进程（主线程）锁，则 panic。
    /// - 若持有多把锁，则 panic。
    /// - 若进程（主线程）状态为运行中，尝试切换会 panic。
    /// - 若中断未关闭，则 panic。
    ///
    /// # 安全性
    ...
    /// # 功能说明
    /// 尝试让当前 CPU 上持有的进程（主线程）主动让出 CPU 执行权（yield）。
    /// 如果当前 CPU 没有进程（主线程）或进程（主线程）不处于运行状态，则直接返回。
    ///
    /// # 流程解释
    /// 1. 检查当前 CPU 的 `proc` 指针是否为空，判断是否有进程（主线程）存在。
    /// 2. 如果存在进程（主线程），获取该进程（主线程）的排他锁 `excl`。
    /// 3. 判断进程（主线程）状态是否为 `RUNNING`。
    ///    - 若是，释放锁后调用进程（主线程）的 `yielding()` 方法，触发主动让出。
    ///    - 否则，直接释放锁，函数返回。
    ///
    /// # 参数
    ...
    /// - 若进程（主线程）锁 `excl` 锁定失败，可能阻塞或引发死锁（取决于锁实现）。
    ///
    /// # 安全性
    /// - `unsafe` 用于裸指针解引用，调用者必须保证 `proc` 指针始终有效且唯一。
    /// - 锁机制保证并发安全，避免进程（主线程）状态被竞态修改。
    ///

...
use crate::process::proc::manager::TaskFIFO;
...
///
/// 这是一个模仿 xv6 操作系统的教学内核中的核心进程（主线程）管理结构体的唯一实例，
/// 用于维护和管理系统中所有进程（主线程）的生命周期、状态和调度相关数据。
///
/// # 用途
/// - 持有固定大小的进程（主线程）表（`table`），包含所有进程（主线程）的状态和资源信息。
/// - 分配新的进程（主线程）标识符（PID）。
/// - 负责进程（主线程）的创建、初始化、唤醒、阻塞、退出和等待等管理操作。
/// - 管理进程（主线程）间的父子关系。
/// - 提供进程（主线程）调度相关的接口，如分配可运行进程（主线程）等。
///
/// # 并发安全性
...
/// 任务管理器（Task Manager）
///
/// 这是一个模仿 xv6 的教学操作系统内核中用于管理所有进程（主线程）的核心结构体，
/// 负责维护系统中所有进程（主线程）的状态、父子关系、PID 分配以及调度相关操作。
///
/// 主要功能包括：
/// - 存储固定数量的进程（主线程）表，每个元素代表一个进程（主线程）。
/// - 管理进程（主线程）的父子关系映射。
/// - 维护初始进程（主线程）索引（通常为第一个进程（主线程））。
/// - 分配并维护全局唯一的进程（主线程）标识符（PID）。
///
...
impl ProcManager {
    ...
    /// # 功能说明
    ///
    /// 初始化进程（主线程）管理器中所有进程（主线程）的内核栈空间。
    /// 具体操作包括为每个进程（主线程）分配一段连续的物理页（4 页大小），
    ...
    /// # 参数
    ///
    /// - `&mut self`：进程（主线程）管理器的可变引用，允许修改进程（主线程）表中进程（主线程）的内核栈地址。
    ///
    /// # 返回值
    ///
    /// - 无返回值。
    ...
    pub unsafe fn proc_init(&mut self) {
        for (pos, task) in self.table.iter_mut().enumerate() {
            ...
            task.data.get_mut().set_kstack(va);
        }
    }

    /// # 功能说明
    ///
    /// 分配一个唯一的进程（主线程）标识符（PID）。
    /// 该函数通过自旋锁保护，支持多线程并发调用，
    /// 确保每次调用返回的 PID 都是唯一且递增的。
    ///
    /// # 参数
    ///
    /// - `&self`：对进程（主线程）管理器的不可变引用，
    ...
    /// # 功能说明
    ///
    /// 在进程（主线程）表中查找一个状态为 `UNUSED` 的空闲进程（主线程）条目，
    /// 如果找到则为该进程（主线程）分配新的 PID，初始化运行内核所需的状态，
    /// 包括分配陷阱帧（trapframe）和页表，
    /// 并将进程（主线程）状态设置为 `ALLOCATED`。
    /// 返回该已分配但尚未运行的进程（主线程）的可变引用。
    ///
    /// # 参数
    ///
    /// - `&mut self`：对进程（主线程）管理器的可变引用，允许修改进程（主线程）表。
    ///
    /// # 返回值
    ///
    /// - 返回 `Some(&mut Proc)`，表示成功分配并初始化了一个进程（主线程）。
    /// - 返回 `None`，表示没有找到可用的空闲进程（主线程），或资源分配失败（如内存不足）。
    ///
    /// # 可能的错误
    ///
    ...
    ///   需要保证底层内存分配正确且有效。
    fn alloc_proc(&mut self) -> Option<&mut Task> {
        for task in self.table.iter_mut() {
            ...
            match guard.state {
                ProcState::UNUSED => {
                    ...
                    debug_assert!(tdata.pagetable.is_none());

                    match PageTable::alloc_proc_pagetable(tdata.trapframe as usize, new_tid) {
                        Some(pgt) => tdata.pagetable = Some(pgt),
                        None => {
                            unsafe {
                                RawSinglePage::from_raw_and_drop(tdata.trapframe as *mut u8);
                            }
                            return None;
                        }
                    }
    ...
    pub fn alloc_task(&mut self, parent: *const Task) -> Option<&mut Task> {
        for task in self.table.iter_mut() {
            let mut guard = task.excl.lock();
            match guard.state {
                ProcState::UNUSED => {
    ...
    /// # 功能说明
    ///
    /// 在进程（主线程）表中查找第一个状态为 `RUNNABLE` 的进程（主线程），
    /// 将其状态修改为 `ALLOCATED`，
    /// 并返回该进程（主线程）的可变引用，
    /// 返回时不持有该进程（主线程）的锁，
    /// 该函数通常由各 CPU 的调度器调用以选取下一个运行的进程（主线程）。
    ///
    /// # 参数
    ///
    /// - `&mut self`：进程（主线程）管理器的可变引用，允许修改进程（主线程）状态。
    ///
    /// # 返回值
    ///
    /// - 返回 `Some(&mut Proc)` 表示找到一个可运行的进程（主线程）并分配成功。
    /// - 返回 `None` 表示当前没有可运行的进程（主线程）。
    ///
    /// # 可能的错误
    ///
    ...
    /// # 安全性
    ///
    /// - 通过持有进程（主线程）的 `excl` 自旋锁保证状态修改的原子性，避免竞态条件。
    /// - 返回时释放了锁，调用者需确保使用该进程（主线程）指针时的并发安全。
    fn alloc_runnable(&mut self) -> Option<&mut Task> {
        for task in self.table.iter_mut() {
            let mut guard = task.excl.lock();
            match guard.state {
                ProcState::RUNNABLE => {
    ...
    ///
    /// 初始化系统的第一个用户进程（主线程）。
    /// 该函数调用 `alloc_proc` 分配一个新的进程（主线程）结构，
    /// 执行该进程（主线程）的用户态初始化逻辑，
    /// 并将其状态设置为 `RUNNABLE`，
    /// 准备被调度执行。
    ///
    /// # 参数
    ///
    /// - `&mut self`：进程（主线程）管理器的可变引用，允许修改进程（主线程）表。
    ///
    /// # 返回值
    ///
    /// - 无返回值。
    ...
    /// # 可能的错误
    ///
    /// - 如果所有进程（主线程）均非 UNUSED 状态，`alloc_proc` 返回 `None`，
    ...
    ///   - 初始进程（主线程）在进程（主线程）表中的索引为 0，
    ///     并且该索引对应的进程（主线程）尚未被占用。
    ...
    /// 检查给定的进程（主线程）是否是init
    fn is_init_proc(&self, p: &Task) -> bool {
        ptr::eq(&self.table[0], p)
    }

    /// # 功能说明
    ///
    /// 唤醒所有阻塞在指定通道 `channel` 上的进程（主线程）。
    /// 遍历进程（主线程）表，查找处于 `SLEEPING` 状态且等待通道为 `channel` 的进程（主线程），
    /// 将它们的状态修改为 `RUNNABLE`，
    /// 使这些进程（主线程）能够被调度器选中运行。
    ///
    /// 注意：调用此函数时，不能持有任何进程（主线程）的锁，以避免死锁。
    ///
    /// # 参数
    ///
    /// - `&self`：对进程（主线程）管理器的不可变引用，用于访问进程（主线程）表。
    /// - `channel: usize`：等待通道标识，唤醒所有阻塞在此通道的进程（主线程）。
    ///
    /// # 返回值
    ///
    /// - 无返回值。
    ...
    /// # 可能的错误
    ///
    /// - 如果在调用此函数时持有了任一进程（主线程）的锁，可能导致死锁。
    /// - 并发情况下，若其他线程同时修改进程（主线程）状态，可能存在竞态风险。
    ///
    /// # 安全性
    ///
    /// - 函数内部对每个进程（主线程）状态修改通过持有进程（主线程）的自旋锁 `excl` 保护，
    ///   保证了修改操作的线程安全。
    /// - 调用者必须保证调用时未持有任何进程（主线程）锁，避免潜在死锁。
    ...
    /// # 功能说明
    ///
    /// 设置指定子进程（主线程）的父进程（主线程）索引。
    /// 将 `child_i` 索引的子进程（主线程）的父进程（主线程）设置为 `parent_i`，
    /// 更新父子关系映射表。
    ///
    /// # 参数
    ///
    /// - `&self`：进程（主线程）管理器的不可变引用，用于访问和修改父子关系映射。
    /// - `child_i: usize`：子进程（主线程）在进程（主线程）表中的索引。
    /// - `parent_i: usize`：父进程（主线程）在进程（主线程）表中的索引。
    ///
    /// # 返回值
    ///
    /// - 无返回值。
    ...
    /// - 调用前若该子进程（主线程）已经有父进程（主线程），则会触发调试断言失败。
    ///
    /// # 安全性
    ///
    ...
    /// # 功能说明
    ///
    /// 使指定进程（主线程）进入退出状态，执行退出清理流程。
    /// 具体包括关闭进程（主线程）打开的文件，
    /// 将其子进程（主线程）的父进程（主线程）重新指向初始进程（主线程），
    /// 唤醒相关父进程（主线程），
    /// 设置退出状态并将进程（主线程）状态改为 `ZOMBIE`，
    /// 最后调用调度器切换进程（主线程）上下文，不返回。
    ///
    /// # 参数
    ///
    /// - `&self`：进程（主线程）管理器的不可变引用，允许访问和修改进程（主线程）相关数据。
    /// - `exit_pi: usize`：要退出的进程（主线程）在进程（主线程）表中的索引。
    /// - `exit_status: i32`：进程（主线程）退出状态码，用于父进程（主线程）查询。
    ///
    /// # 返回值
    ///
    /// - 无返回值。函数内部调用调度器后不会返回，
    ...
    /// # 可能的错误
    ///
    /// - 如果试图退出的是初始进程（主线程）（`init_proc`），会触发 panic，
    ///   因为初始进程（主线程）不允许退出。
    /// - 代码中假设进程（主线程）索引和父子关系合法，
    ///   若数据结构异常可能导致未定义行为。
    ///
    /// # 安全性
    ///
    /// - 调用过程中涉及多处 `unsafe`，
    ///   包括对进程（主线程）数据的裸指针操作和上下文获取，
    ///   需保证对应进程（主线程）数据有效且未被其他线程破坏。
    /// - 持有和释放自旋锁时保证并发安全，
    ///   通过锁保护防止数据竞态。
    /// - 调用调度器切换上下文时，
    ///   确保当前 CPU 和进程（主线程）状态正确，避免死锁或调度异常。
    fn exiting(&self, exit_index: usize, exit_status: i32) {
        ...
        if have_child {
            self.wakeup(&self.table[self.init_proc] as *const Task as usize);
        }
        let exit_parenti = *parent_map[exit_index].as_ref().unwrap();
        self.wakeup(&self.table[exit_parenti] as *const Task as usize);
    ...
    /// # 功能说明
    ///
    /// 父进程（主线程）等待其任一子进程（主线程）退出（进入 ZOMBIE 状态）。
    /// 如果找到已退出的子进程（主线程），将子进程（主线程）的退出状态复制到用户空间，
    /// 清理子进程（主线程）资源，解除父子关系映射，
    /// 并返回该子进程（主线程）的 PID。
    /// 如果没有子进程（主线程）或调用进程（主线程）被杀死，则返回错误。
    /// 若存在子进程（主线程）但均未退出，调用进程（主线程）将进入睡眠状态，
    /// 直到被唤醒重新检测。
    ///
    /// # 参数
    ///
    /// - `&self`：进程（主线程）管理器的不可变引用，用于访问进程（主线程）表和父子关系。
    /// - `pi: usize`：调用该函数的父进程（主线程）在进程（主线程）表中的索引。
    /// - `addr: usize`：用户空间地址，
    ///   若非零，退出状态将复制到该地址。
    ///
    /// # 返回值
    ///
    /// - `Ok(usize)`：返回已退出子进程（主线程）的 PID。
    /// - `Err(())`：表示没有子进程（主线程）可等待，或者调用进程（主线程）被杀死。
    ///
    /// # 可能的错误
    ///
    /// - 如果用户空间地址无效或拷贝失败，返回错误。
    /// - 如果无子进程（主线程）或调用进程（主线程）已被杀，返回错误。
    ///
    /// # 安全性
    ///
    ...
    /// - 调用 `sleep` 使调用进程（主线程）阻塞，等待唤醒重新检测，
    ///   需保证唤醒机制和锁释放顺序正确避免死锁。
    fn waiting(&self, pi: usize, addr: usize) -> Result<usize, ()> {
        ...
        loop {
            ...
            if !have_child || task.killed.load(Ordering::Relaxed) {
                return Err(());
            }
    ...
    /// # 功能说明
    ///
    /// 根据给定的进程（主线程）标识符（PID）杀死对应的进程（主线程）。
    /// 查找进程（主线程）表中匹配的进程（主线程），将其 `killed` 标记置为 `true`，
    /// 如果该进程（主线程）处于 `SLEEPING` 状态，则将其状态改为 `RUNNABLE`，
    /// 以便尽快响应终止请求。
    ///
    /// # 参数
    ///
    /// - `&self`：进程（主线程）管理器的不可变引用，用于访问进程（主线程）表。
    /// - `pid: usize`：要杀死的进程（主线程）的 PID。
    ///
    /// # 返回值
    ///
    /// - `Ok(())` 表示成功找到并标记了该进程（主线程）为被杀死。
    /// - `Err(())` 表示未找到指定 PID 的进程（主线程）。
    ///
    /// # 可能的错误
    ///
    ...
    /// # 安全性
    ///
    /// - 函数通过进程（主线程）的自旋锁 `excl` 保护对进程（主线程）状态的修改，保证并发安全。
    /// - 标记进程（主线程）为被杀死后，依赖其他机制（如调度器或系统调用）
    ///   处理后续清理和终止动作。
...
/// fork 创建的子进程（主线程）首次被调度器调度时，
/// 会切换到 forkret 函数执行。
/// 需要小心处理，因为 CPU 会使用返回地址寄存器（ra）跳转到这里。
///
/// 安全性说明1：该函数应仅由第一个用户进程（主线程）调用，
/// 之后其他用户进程（主线程）可以并发调用 fork_ret。
/// 安全性说明2：该函数是非可重入的，
/// 中断或异常处理程序不得调用此函数。
unsafe fn fork_ret() -> ! {
...
/// # 功能说明
///
/// 根据进程（主线程）索引 `pos` 计算该进程（主线程）的内核栈虚拟地址起始位置。
/// 内核栈在虚拟地址空间中按顺序从高地址向低地址分配，
/// 每个进程（主线程）分配连续的 5 页空间（包含实际栈页和保护页）。
/// 该函数返回内核栈的最高地址（栈顶）虚拟地址，用于内核栈映射和访问。
///
/// # 参数
///
/// - `pos: usize`：进程（主线程）在进程（主线程）表中的索引，用于计算内核栈偏移。
///
/// # 返回值
///
/// - 返回 `usize` 类型的虚拟地址，表示对应进程（主线程）内核栈的起始地址（栈顶地址）。
#[inline]
